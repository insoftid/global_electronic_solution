  ...
  <div class="grid grid-cols-1 lg:grid-cols-12 gap-4">
          ...
          <tbody id="msgBody">
            @foreach($messages as $m)
              <tr class="msg-row cursor-pointer border-b border-gray-50 hover:bg-gray-50 transition"
                data-id="{{ $m['id'] }}" data-name="{{ $m['name'] }}" data-email="{{ $m['email'] }}"
                data-subject="{{ $m['subject'] }}" data-status="{{ $m['status'] }}" data-date="{{ $m['date'] }}"
    ...
    <div class="lg:col-span-5 bg-white border border-gray-200 rounded-2xl overflow-hidden flex flex-col">
      ...
      <div class="border-t border-gray-100 px-4 py-4 flex justify-end gap-2 mt-auto">
        <button id="markReadBtn" type="button"
...
<script>
  (function () {
    ...
    function updateMarkReadButton(status) {
      if (status === 'Dibaca') {
        markReadBtn.textContent = 'Tandai Belum Dibaca';
        markReadBtn.className = 'rounded-full bg-orange-200 px-4 py-2 text-sm text-orange-800 hover:bg-orange-300';
      } else {
        markReadBtn.textContent = 'Tandai Dibaca';
        markReadBtn.className = 'rounded-full bg-gray-200 px-4 py-2 text-sm text-gray-800 hover:bg-gray-300';
      }
    }
    ...
    markReadBtn.addEventListener('click', () => {
      ...
      const newStatus = currentStatus === 'Dibaca' ? 'Baru' : 'Dibaca';
      ...
      setBadge(newStatus);
      updateMarkReadButton(newStatus);
      applyFilter();
    });
