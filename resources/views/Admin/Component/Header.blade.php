...
@else
    ...
    @endif
        ...
        @if(isset($active) && $active === 'dashboard')
            <div class="flex items-center gap-3">
                <a href="{{ route('admin.contacts.index') }}" class="bg-gray-200 p-2 rounded-lg relative">
                    ...
                    @if(isset($unreadContactCount) && $unreadContactCount > 0)
                        <span
                            class="absolute -right-1 -top-1 bg-red-500 text-white rounded-full px-1.5 py-0.5 text-[10px] min-w-[18px] text-center border-2 border-white">
                            {{ $unreadContactCount }}
                        </span>
                    @endif
                </a>
