{{-- SERTIFIKASI (partial) --}}
@php
  /* $certs should be provided by the parent view */
@endphp
    ...
    <div class="lg:col-span-5 bg-white border border-gray-200 rounded-2xl p-4">
      ...
      <form class="mt-4 space-y-3" onsubmit="return false;">
        ...
        <div>
            ...
            <div id="certFilePreview" class="h-24 bg-gray-50 rounded-md overflow-hidden mb-3 flex items-center justify-center text-gray-400 preview-area">
              <span class="preview-placeholder">Preview Foto</span>
            </div>
            <label for="certFile" class="my-2 w-full h-9 rounded-lg border border-gray-300 inline-flex justify-between items-center cursor-pointer pl-3">
                <span class="text-gray-900/60 text-sm font-normal leading-snug truncate file-name">No file chosen</span>
                <input id="certFile" type="file" name="photos[logo]" accept="image/*" class="hidden file-input" />
            ...
            <p class="text-[10px] text-graytext" id="certFileInfo">SVG, PNG or JPG (Max 2 MB).</p>
        </div>
...
(function(){
  ...
  (function(){
    ...
    const certPreview = document.getElementById('certFilePreview');
    ...
    function fill(row){
      ...
      if(certPreview){ certPreview.innerHTML = ''; if(row.dataset.file){ var img = document.createElement('img'); img.src = row.dataset.file; img.alt = row.dataset.name || 'Sertifikat'; img.className = 'w-full h-full object-contain'; img.style.cursor = 'zoom-in'; img.addEventListener('click', function(){ var modal = document.getElementById('image-modal-landing'); var modalImg = document.getElementById('image-modal-img-landing'); if(modal && modalImg){ modalImg.src = this.src; modal.classList.remove('hidden'); document.body.style.overflow = 'hidden'; modal.setAttribute('aria-hidden','false'); } }); certPreview.appendChild(img); } else { certPreview.innerHTML = '<span class="preview-placeholder">Preview Foto</span>'; } }
    }
    ...
    if(btnReset) btnReset.addEventListener('click', () => {
      ...
      try{
        if(fileEl) fileEl.value = '';
        if(certPreview){
          const prev = certPreview.getAttribute('data-object-url');
          if(prev){ try{ URL.revokeObjectURL(prev); }catch(e){} certPreview.removeAttribute('data-object-url'); }
          certPreview.innerHTML = '<span class="preview-placeholder">Preview Foto</span>';
        }
    ...
    if(btnSave) btnSave.addEventListener('click', () => { alert('UI saja: Simpan sertifikasi belum terhubung ke database.'); });

    if(btnDelete) btnDelete.addEventListener('click', () => { if(!idEl || !idEl.value){ alert('Pilih sertifikasi dulu dari tabel sebelum menghapus.'); return; } const row = document.querySelector(`.cert-row[data-id="${idEl.value}"]`); if(row) row.remove(); if(btnReset) btnReset.click(); alert('UI saja: sertifikasi dihapus dari tampilan.'); });

    if(rows[0]){ setActive(rows, rows[0]); fill(rows[0]); }

    // cert file input preview behavior (scoped)
    if(fileEl){ fileEl.addEventListener('change', function(){ var f = this.files && this.files[0]; if(!certPreview) return; var prev = certPreview.getAttribute('data-object-url'); if(prev){ try{ URL.revokeObjectURL(prev); }catch(e){} certPreview.removeAttribute('data-object-url'); } if(f){ var url = URL.createObjectURL(f); certPreview.innerHTML = ''; var img = document.createElement('img'); img.src = url; img.alt = f.name || 'Preview'; img.className = 'w-full h-full object-contain'; img.style.cursor = 'zoom-in'; img.addEventListener('click', function(){ var modal = document.getElementById('image-modal-landing'); var modalImg = document.getElementById('image-modal-img-landing'); if(modal && modalImg){ modalImg.src = this.src; modal.classList.remove('hidden'); document.body.style.overflow = 'hidden'; modal.setAttribute('aria-hidden','false'); } }); certPreview.appendChild(img); certPreview.setAttribute('data-object-url', url); if(infoEl) infoEl.textContent = f.name; } else { certPreview.innerHTML = '<span class="preview-placeholder">Preview Foto</span>'; if(infoEl) infoEl.textContent = 'Belum ada file dipilih.'; } }); }
