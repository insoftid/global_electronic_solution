...
<nav id="site-navbar" class="fixed bg-white/50 backdrop-blur-lg top-0 left-0 right-0 w-full z-50 transition-all">
	...
	<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
		<div class="flex justify-between h-16 items-center">
				...
				<a href="/" class="flex items-center gap-3">
					@if(isset($settings['logo']) && $settings['logo'])
						<img src="{{ asset('storage/' . $settings['logo']) }}" alt="Logo"
							class="w-8 h-8 rounded-md shadow-sm object-contain">
					@else
					...
					@endif
					<span
						class="text-primary font-bold text-lg">{{ $settings['company_name'] ?? 'Global Electronic Solution' }}</span>
				</a>
			</div>
			...
			<div class="hidden md:flex items-center space-x-6">
				<a href="/#tentang"
					class="nav-link text-md text-black hover:text-primary {{ (isset($active) && ((is_array($active) && in_array('tentang', $active)) || $active === 'tentang')) ? 'active server-active' : '' }}">Tentang
					Kami</a>
				<a href="/produk"
					class="nav-link text-md text-black hover:text-primary {{ (isset($active) && ((is_array($active) && in_array('portofolio', $active)) || $active === 'portofolio')) ? 'active server-active' : '' }}">Produk</a>
				<a href="/contact"
					class="nav-link text-md text-black hover:text-primary {{ (isset($active) && ((is_array($active) && in_array('kontak', $active)) || $active === 'kontak')) ? 'active server-active' : '' }}">Kontak</a>
			</div>

			<!-- Mobile menu button -->
			<div class="md:hidden">
				<button id="nav-toggle"
					class="inline-flex items-center justify-center p-2 rounded-md text-gray-500 hover:text-gray-700 focus:outline-none">
	...
	<div id="nav-menu" class="md:hidden hidden border-t border-gray-100">
		...
		<div class="px-4 pt-2 pb-3 flex flex-col items-start space-y-3">
			<a href="#tentang"
				class="nav-link inline-block text-md text-black hover:text-primary {{ (isset($active) && ((is_array($active) && in_array('tentang', $active)) || $active === 'tentang')) ? 'active server-active' : '' }}">Tentang
				Kami</a>
			<a href="/produk"
				class="nav-link inline-block text-md text-black hover:text-primary {{ (isset($active) && ((is_array($active) && in_array('portofolio', $active)) || $active === 'portofolio')) ? 'active server-active' : '' }}">Portofolio</a>
			<a href="/contact"
				class="nav-link inline-block text-md text-black hover:text-primary {{ (isset($active) && ((is_array($active) && in_array('kontak', $active)) || $active === 'kontak')) ? 'active server-active' : '' }}">Kontak</a>
		</div>
	</div>
